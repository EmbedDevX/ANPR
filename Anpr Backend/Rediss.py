...
class Send_data:
	...
	def __init__(self,redis_host,redis_port,connection=0):
		...
		while True :
			try :
				...
				logger.error("Unable to connect with the redis server")
	...
	def check_redis_connection(self):
		if self.connection==True:
			...
			logger.error("Redis Server is not working Properly")
			while True :
				try:
					...
					logger.error("Unable to Connect Redis Server with exception "+str(e))
