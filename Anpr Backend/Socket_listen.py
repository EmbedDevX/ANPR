...
#red = Rediss.Send_data('localhost',6379)
...
class Camera:
	def __init__(self,tcp_ip,tcp_port,camera_location):
		...
		while True :
			...
			try:
				...
				logger.info("There is some error in creating socket with error message"+str(e))
		...
		try:
			...
			while True:
				try:
					...
					logger.info("There is some error in creating socket connection with exception"+str(e))
	...
	def check_db_status(self):
		try:
			...
			logger.info("Unable to check database connectivity with exception"+str(e))
			print(str(e))
