...
class Database :
    ...
    def check_vehicle_details(self,data):
        ...
        if data == None :
            ...
            if number_plate ==None :
                ...
                row = cursor.fetchone()
                print("row result is ",row[0])
                if row[0] == 0:
                    cursor.execute("select count(*) from guestEntry where vehicleNumber=(%s)",number_plate)
    ...
    def insert_into_activity(self,data):
        ...
        if data == None :
            ...
            cursor.execute("Insert into activity(vehicleNumber,entryLocation,passType,verificationStatus,entryDate,entryTime)values(%s,%s,%s,%s,%s,%s)",(data["Number Plate"],data["Camera_location"],data["Pass Type"],data["Verification Status"],data["Date"],data["Time"]))
    ...
    def check_pass_type(self,data):
        ...
        if data == None :
            ...
            if number_plate == None :
                ...
                cursor.execute("select passType from gatePass INNER JOIN vehicleDetails ON vehicleDetails.vehicleNumber=gatePass.vehicleNumber WHERE gatePass.vehicleNumber=(%s)",number_plate)#code to check pass type of the vehicle number plate
    ...
    def check_database_connection(self):
        try:
            ...
            logger.error("Database is not working Properly, Exception message"+str(e))
